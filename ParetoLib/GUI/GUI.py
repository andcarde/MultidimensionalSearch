...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def run_parametric_stle(self):
        ...
        try:
            ...
            ROOT_GUI.logger.debug(intervals)
            if len(intervals) < 2:
                raise ValueError('Invalid number of dimensions. Must have at least 2.')

            # Mining the STLe expression
            if method == 0:
                ROOT_GUI.logger.debug('Using method 0.')
                self.oracle = OracleSTLeLib(stl_prop_file, csv_signal_file, stl_param_file)
                ...
                # Popup window for reading the parameters "bound_on_count" and "intvl_epsilon" for Oracle1 and Oracle2
                ROOT_GUI.logger.debug('Using method 1.')
                stl_prop_file = self.spec_filepaths[0]
            ...
            ROOT_GUI.logger.error(f'Unexpected error: {str(e)}')
        finally:
