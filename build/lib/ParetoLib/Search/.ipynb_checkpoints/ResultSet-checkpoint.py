...
class ResultSet(object):
    ...
    def _plot_yup_3D(self, xaxe=0, yaxe=1, zaxe=2, opacity=1.0, clip_box=None):
        ...
        faces = [rect.plot_3D('green', xaxe, yaxe, zaxe, opacity, clip_box) for rect in self.yup if self.champion == None or rect != self.champion]
        if self.champion != None:
            faces.append(rect.plot_3D('blue',xaxe, yaxe, zaxe, opacity, clip_box))
        return faces
    ...
    def select_champion(self, rs_list):
        ...
        self.champion = self.yup[current_class.index(vertex_champinon)]
